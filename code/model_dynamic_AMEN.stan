...
model {
  ...
  for(n in 1:N){
    ...
    Y[n] ~ bernoulli(Phi(  intercept +b_weight*past_weight[n] + b_reci*past_reci[n]+ b_trans*past_trans[n] + mean_nodes[sender_id[n],1] + mean_nodes[receiver_id[n],2] + mean_dyads[dyad_id[n], send_receive[n]] + mean_multi_effects[sender_id[n],1:K] * (mean_multi_effects[receiver_id[n],(K+1):(K*2) ] )' )) ;
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    real p = Phi(  intercept +b_weight*past_weight[n] + b_reci*past_reci[n]+ b_trans*past_trans[n] + mean_nodes[sender_id[n],1] + mean_nodes[receiver_id[n],2] + mean_dyads[dyad_id[n], send_receive[n]] + mean_multi_effects[sender_id[n],1:K] * (mean_multi_effects[receiver_id[n],(K+1):(K*2) ] )' ) ;
    Y_sim[n] = p;
